...
- The tensorflow implementation is:
  ...
  ```
- When we have a large catalogue, we might have a bottleneck because we have to run this huge forward propagation through the two neural network to make inference for all the large catalogue involved, the way to fix this is to use a two-stage architecture where the first is **Retrieval** and this generate some plausible candidates using methods that are fast and approximate such as top movies in user region, ... and we can combine several of these retrieval methods to end up with about
- ---
- ## Connection
- N/A
- ---
