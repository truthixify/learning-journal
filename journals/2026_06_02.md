...
- where $v_u^{(j)}$ is a vector computed from $x_u^{(j)}$ and $v_m^{(i)}$ is a vector computed from $x_m^{(j)}$($x_u^{(j)}$ and $x_m^{(i)}$ . Both v's must have the same dimension for the dot product to work and the way we do do this is that we pass the $x_u^{(j)}$ and $x_m^{(i)}$ into a neural network
- ---
- ## Connection
- N/A
- ---
